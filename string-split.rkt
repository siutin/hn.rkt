#lang racket
(provide string-split-by-length)

(define (partition s #:size [size 10])
  (let ([size (if (> (string-length s) size) size (string-length s))])
   (list
     (substring s 0 size)
     (substring s size (string-length s)))))

(define (string-split-by-length str #:size [size 10])  
  (let* ([item (partition str #:size size)]
         [a (car item)]
         [b (cadr item)])
    (if (= (string-length b) 0)
        (list a)
        (append (list a) (string-split-by-length b #:size size)))))
...
;(string-split-by-length s #:size 300)